...
socket.on('startGame', (ref) => {

    isReferee = ref === socket.id

    if (isReferee) console.log("I am referee");
    startGame()
})

socket.on("paddleMove", (paddleData) => {
    const otherGuy = 1 - paddleIndex
    const { xPos } = paddleData

    paddleX[otherGuy] = xPos
})
